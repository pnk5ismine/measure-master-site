/* /js/mm-auth.js : 모든 페이지가 공유하는 인증/세션 유틸 */
(function(global){
  // --- Supabase 클라이언트 ---
  ...
  // 다른 탭에서 로그인/로그아웃 → storage 이벤트로 동기화
  ...
  function isAdmin(session){ const email = session?.user?.email || ''; return ADMIN_EMAILS.includes(email); }
